-- Transformation Pulse Global: Supabase Schema
-- Run this in Supabase SQL Editor to create tables with RLS
...
-- Stakeholders: recruiters, hiring managers, bridges (mutual connections)
...
CREATE TABLE IF NOT EXISTS stakeholders (
  ...
  firm_name TEXT,
  -- For bridges: Relationship Strength Score
...
-- Row Level Security (RLS): enable for private access
ALTER TABLE signals ENABLE ROW LEVEL SECURITY;
ALTER TABLE stakeholders ENABLE ROW LEVEL SECURITY;

-- Policy: allow authenticated users only (adjust for your auth strategy)
CREATE POLICY "Allow authenticated read signals" ON signals
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated insert signals" ON signals
  FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "Allow authenticated update signals" ON signals
  FOR UPDATE TO authenticated USING (true);

CREATE POLICY "Allow authenticated read stakeholders" ON stakeholders
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated insert stakeholders" ON stakeholders
  FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "Allow authenticated update stakeholders" ON stakeholders
  FOR UPDATE TO authenticated USING (true);

-- Optional: service role can bypass RLS for cron/backend jobs
-- Use Supabase service_role key for scraper/cron; use anon/authenticated for UI

COMMENT ON TABLE signals IS 'Job mandates from regional portals; one row per detected signal';
COMMENT ON TABLE stakeholders IS 'Power Triangle: Recruiter, Hiring Manager, Bridges (mutual connections with RSS)';

-- =============================================================================
-- SELF-HEALING & INTEGRATION (Phoenix Rebirth)
-- =============================================================================

-- Selectors / Config: AI-updatable extraction rules per source (site + field)
...
-- Scrape sources: status for dashboard (Green / Orange / Blue)
...
-- Heal events: before/after logic for observability (e.g. Arize Phoenix)
...
COMMENT ON TABLE scraper_selectors IS 'AI-updatable selectors when site structure changes (Phoenix Rebirth)';
COMMENT ON TABLE scrape_sources IS 'Per-source status: ok | healing | healed for dashboard indicators';
COMMENT ON TABLE heal_events IS 'Before/after selector changes for audit and Arize Phoenix';

-- =============================================================================
-- STEALTH GOVERNOR & ANTI-BAN (Shadow Pillar)
-- =============================================================================

-- Session persistence: store browser cookies/context so scrapers reuse sessions
-- and avoid "Suspicious Login" alerts. Do not re-login for every scrape.
...
-- Regional profiles: proxy + timezone per hub (Safe Hub launch strategy)
...
COMMENT ON TABLE browser_sessions IS 'Stealth: reuse sessions to avoid new-login alerts';
COMMENT ON TABLE regional_profiles IS 'Per-hub timezone + proxy for Safe Hub strategy';

-- User professional history (editable my_history; local-first / Supabase)
...
COMMENT ON TABLE user_professional_history IS 'Editable my_history per user; used for Bridge overlap + anonymized LLM';
